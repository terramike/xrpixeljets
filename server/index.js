// index.js — XRPixel Jets API (2025-10-26cfg1)
...
await app.register(cors, {
  origin: CORS_ALLOW, methods:['GET','POST','OPTIONS'],
  allowedHeaders:['Content-Type','Accept','Origin','X-Wallet','Authorization'],
  preflight:true, hideOptionsRoute:false
});
...
app.addHook('onRequest', async (req, reply) => {
  if (req.raw.url.startsWith('/session/start') || req.raw.url.startsWith('/config')) return;
...
async function applyRegen(wallet,row){
  try {
    ...
    if (minutes>0 && regen>0) {
      ...
      const next = clamp((row.energy|0)+minutes*regen, 0, cap);
...
function missionReward(level){ const l=Math.max(1,Number(level)||1); if(l<=5) return [0,100,150,200,250,300][l]; return clamp(Math.round(300*Math.pow(1.01,l-5)),1,10000); }
...
app.get('/ms/costs', async (req, reply) => {
  ...
  if (!row) return reply.code(404).send({ error:'not_found' });
  reply.send({ costs: calcCosts(levelsFromRow(row), scale), levels: levelsFromRow(row), scale });
});
...
app.post('/claim/start', async (req, reply) => {
  ...
  try {
    const sent = await claim.sendIssued({ to:req.wallet, amount: amt }); // may throw trustline/inventory errors
