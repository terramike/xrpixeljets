...
const CLAIM_SHARED_SECRET =
  process.env.PAYOUT_SERVICE_SHARED_SECRET ||
  process.env.CLAIM_SHARED_SECRET ||
  process.env.CLAIM_SERVICE_SHARED_SECRET ||
  '';
...
function requireClaimAuth(req, reply) {
  const authHeader = String(req.headers['authorization'] || '');
  const bearer = authHeader.startsWith('Bearer ') ? authHeader.slice(7).trim() : '';
  const claimSecret = String(req.headers['x-claim-secret'] || '').trim();

  if (CLAIM_SHARED_SECRET) {
    const provided = bearer || claimSecret;
    if (provided && secureEquals(provided, CLAIM_SHARED_SECRET)) {
      return { mode: 'shared_secret' };
    }
  }

  return requireJWT(req, reply);
}
...
app.post('/claim/start', async (req, reply) => {
  ...
  if (walletAddress && walletAddress !== req.wallet) {
  ...
  try {
    ...
    return reply.code(500).send({ error: 'claim_failed', requestId });
  }
});
