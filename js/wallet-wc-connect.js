...
(function attach(g) {
  ...
  const WEB_BASE = String(g.JETS_WEB_BASE || 'https://mykeygo.io/jets').replace(/\/+$/, '');
  const ICON_URL = g.JETS_ICON_URL || `${WEB_BASE}/assets/favicon.png`;
  const API_BASE = String(g.JETS_API_BASE || 'https://xrpixeljets.onrender.com').replace(/\/+$/, '');
