...
const REG_URLS = [
  '/jets/registry.json',
  'https://mykeygo.io/jets/registry.json'
];
...
function ipfsCandidates(uri) {
  let id = String(uri || '').replace(/^ipfs:\/\//, '').replace(/^ipfs\//, '');
  id = id.trim(); // Remove trailing/leading spaces from metadata
  return IPFS_GW.map(g => g + id);
}

function candidatesFor(uri) {
  if (!uri) return [];
  const s = String(uri).trim(); // Clean trailing spaces
...
async function parseGearFromMeta(nft) {
  ...
  const img = (() => {
    const u = (j.image || j.image_url || props.image || j.video || '').trim();
    if (!u) return '';
