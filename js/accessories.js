/* /jets/js/accessories.js — OG inline + Registry Crew (issuer-aware)
   ...
   - Issuer/taxon matching from registry entries (exact Issuer match; optional NFTokenTaxon allowlist)
   - Keeps URL/text "match" fallback
   - Totals = registry sums + gear sums (unchanged)
   - NEW: Jets (taxon 200) are ignored by gear parser so Legendary Jets don't show as accessories
*/
...
const REG_URLS = [
  '/jets/registry.json?v=2025-11-13r2',
  'https://mykeygo.io/jets/registry.json?v=2025-11-13r2',
  'https://www.mykeygo.io/jets/registry.json?v=2025-11-13r2'
];
  ...
  for (const url of REG_URLS){
    try{
      ...
      if (r.ok){
        ...
        REGISTRY_TS = now;
        console.info('[Accessories] Registry loaded:',url);
        return REGISTRY;
      }
...
/* --- Upgrade parser for in-game gear (with Jet skip) --- */
async function parseGearFromMeta(nft){
  // Skip XRPixel Jets themselves so they don't become "gear"
  ...
  if (!stat && props.stat != null) {
    stat = String(props.stat).toLowerCase();
  }
  if (!Number.isFinite(bonus) && props.bonus != null) {
    bonus = Number(props.bonus);
  }

  // Direct stat/bonus attributes
  if ((!stat || !Number.isFinite(bonus)) && Array.isArray(j.attributes)) {
    for (const a of j.attributes) {
      ...
      const v = a.value;
      if (!stat && k === 'stat' && v != null) {
        stat = String(v).toLowerCase();
      }
      if (!Number.isFinite(bonus) && k === 'bonus' && v != null) {
        bonus = Number(v);
      }
    }
  }

  // Fallback for Bazaar-style upgrades: infer from Attack/Defense/Speed attrs
  ...
  if (needFallback) {
    ...
    if (Array.isArray(j.attributes)) {
      ...
      if (typeof atts['kind'] === 'string') {
        kind = String(atts['kind']).toLowerCase().trim();
      }
    }
    ...
    if (kind && CANDS.includes(kind) && Number.isFinite(nums[kind]) && nums[kind] > 0) {
      ...
      for (const k of CANDS) {
        if (Number.isFinite(nums[k]) && nums[k] > 0) {
          chosen = k;
          break;
        }
      }
...
/* --- Registry detection: issuer/taxon aware --- */
...
function aggregateRegistry(items){
  ...
  for (const { bonuses } of byColl.values()) {
    for (const k of Object.keys(registryTotals)) {
      registryTotals[k] += Number(bonuses[k] || 0); // sum across collections
    }
  }
